class Parallelogram {
  constructor(ctx, seq, w = 70, h = 70, leftTopX = 100, leftTopY = 100, color = "#009900", W, H, answer) {
    ...
    this.reverseIconW = W / 12;
    this.reverseIconH = W / 20;
  ...
  render() {
    if(this.reverse) {
      this.tangram = [
        {
          p: [
            ...
            {
              x: (this.rotateX + this.leftTopX - (this.rotateY - this.leftTopY)) 
                - ((this.leftTopX - (this.rotateY - this.leftTopY)) + (2 * this.leftTopX - this.rotateX)) / 2,
              y: (this.rotateY + this.rotateX + (this.leftTopY - this.leftTopX)) 
                - ((this.rotateX + (this.leftTopY - this.leftTopX)) + (2 * this.leftTopY - this.rotateY)) / 2
            },
            {
              x: (this.rotateX + this.leftTopX - (this.rotateY - this.leftTopY)) / 2,
              y: (this.rotateY + this.rotateX + (this.leftTopY - this.leftTopX)) / 2
            }
          ],
          color: this.color
        },
      ...
      this.reverseShape = [
        ...
        , { x: this.leftTopX + this.reverseIconW, y: this.leftTopY }
        , { x: this.leftTopX , y: this.leftTopY + this.reverseIconH}
        , { x: this.leftTopX - this.reverseIconW, y: this.leftTopY + this.reverseIconH }        
      ]    
    } else {
      this.tangram = [
        {
          p: [
            ...
            {
              x: Math.round((this.rotateX + this.leftTopX - (this.rotateY - this.leftTopY)) / 2),
              y: Math.round((this.rotateY + this.rotateX + (this.leftTopY - this.leftTopX)) / 2)
            },
            {
              x: Math.round(((this.leftTopX - (this.rotateY - this.leftTopY)) + (2 * this.leftTopX - this.rotateX)) / 2),
              y: Math.round(((this.rotateX + (this.leftTopY - this.leftTopX)) + (2 * this.leftTopY - this.rotateY)) / 2)
            }
          ],
          color: this.color
        },
    ...
    if ((this.touchedUnit || this.touchedRotate) && !this.touchMove) {
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(this.reverseShape[0].x, this.reverseShape[0].y);
      for (var j = 1; j < this.reverseShape.length; j++) {
        this.ctx.lineTo(this.reverseShape[j].x, this.reverseShape[j].y);
      }
      this.ctx.closePath();
      this.ctx.fill();    
      this.ctx.stroke();
  ...
  touchStartEvent(x, y) {
      ...
      if (this.checkFingerOnReverse(x, y)) {
  ...
  checkFingerOnReverse(x, y) {
    // if (this.checkFingerOnRotate(x, y) || this.checkFingerOnUnit(x, y)) {
    //   return false;
    // } else {
      return this.isInSquare(this.reverseShape[0], this.reverseShape[1], 
        this.reverseShape[2], this.reverseShape[3], { x: x, y: y }, this.reverseIconW, this.reverseIconH);
    // }
  }
